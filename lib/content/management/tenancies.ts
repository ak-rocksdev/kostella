 ...
 *
 * EVERY RECORD BELOW IS INVENTED, and this repository is public.
 *
 * The names are Indonesia's stock example names — Budi, Siti, Dewi, Bambang —
 * the ones that fill every form sample and textbook exercise. An Indonesian
 * reader takes them as "contoh" the way an English one takes John Doe, so they
 * read as people on screen without being mistakable for Kostella's actual
 * tenants at a real address in Grogol. Phone numbers stay masked.
 *
 * They were "Penghuni A" through "Penghuni V" first. That was safe and looked
 * like an unfinished wireframe, which is the wrong trade for the one screen a
 * client is shown. A real deployment replaces the whole array; nothing derives
 * from the names.
 */
...
const SEED: Seed[] = [
  ...
  },
  {
    ...
    building: '18',
    room: '103',
    name: 'Yudi Firmansyah',
    phone: '0813 xxxx 5580',
    guardianName: 'Bu Firmansyah',
  ...
  //    the notice above — the state this phase exists to make visible. ────
  {
    ...
    monthsHere: 0,
    agreedRent: 1_950_000,
    note: 'Menggantikan Putri Maharani',
  },
]
